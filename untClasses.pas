...
begin
  ...
  try
    ...
    LocQuery.SQL.Text := 'INSERT INTO VENDA (NumeroPedido, DataEmissao, CodigoCliente, ValorTotal) '+
                         ...
                         QuotedStr(FormatarDataSQL(Self.getDataEmissao)) + ', ' +
                         LocCodigoCliente + ', ' +
                         FormatFloat('#0.00',Self.getValorTotal).Replace('.','').Replace(',','.') + ')';
    LocQuery.ExecSQL();
    ...
    begin
      ...
      LocQuery.SQL.Text := 'INSERT INTO ITEMVENDA (CodigoProduto, NumeroPedido, '+
           ...
           FloatToStr(LocItemPedido.Quantidade).Replace('.','').Replace(',','.') + ', ' +
           FloatToStr(LocItemPedido.PrecoVenda).Replace('.','').Replace(',','.') + ', ' +
           FloatToStr(LocItemPedido.ValorTotalItem).Replace('.','').Replace(',','.') + ')';
      LocQuery.ExecSQL();
    end;
